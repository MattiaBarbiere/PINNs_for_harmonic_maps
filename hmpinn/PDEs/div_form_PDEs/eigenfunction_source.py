...
def f(x, a=1, b=1, amplitude=1, backend=torch):
    ...
    amplitude (int): The amplitude of the source term (default 1)
...
def u(x, a=1, b=1, amplitude=1,  backend=torch):
    ...
    amplitude (float): Amplitude of the solution (default = 1)
    ...
    return amplitude * (1 / ((backend.pi / a)**2 + (backend.pi / b)**2)) * backend.sin(backend.pi * x[:, 0] / a) * backend.sin(backend.pi * x[:, 1] / b)
...
class EigenfunctionSourceDF(DivFormResidual, ConstantBC, WithSolution):
    def __init__(self, a=1, b=1, amplitude=1, backend=torch):
        """
        An instance of a Poisson equation when the source term is an eigenfunction in divergence form
