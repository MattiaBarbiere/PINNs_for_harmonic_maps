"""
Non-Symmetric Diffusion PDE.

This module provides a non-symmetric diffusion PDE implementation with known analytical solution.
The PDE features a non-symmetric diffusion matrix in divergence form.

Classes:
    NonSymDiffusion: Non-symmetric diffusion PDE with analytical solution.
"""
...
def f(x, backend=torch):
    """
    Source term function for the non-symmetric diffusion equation.
    ...
    # Compute source term expression for non-symmetric diffusion
    result = (2 * x_val - 6 * x_val**2) * (y_val - y_val**2) + \
            (1 - 2 * y_val) * (2 * x_val - 3 * x_val**2) + \
            (1 - 2 * x_val) * (3 * y_val**2 - 4 * y_val**3) + \
            (1 - 4 * y_val) * (x_val - x_val**2)
    return result

def u(x, backend=torch):
    """
    Analytical solution for the non-symmetric diffusion equation.
...
def diffusion_matrix(x, model=None, backend=torch):
    """
    Non-symmetric diffusion matrix function.
    ...
    Returns:
        torch.Tensor or fn.Array
            Non-symmetric diffusion matrix of shape (batch_size, 2, 2) or symbolic array.
    """
    # Handle the symbolic case 
...
class NonSymDiffusion(DivFormResidual, ConstantBC, WithSolution):
    """
    Non-symmetric diffusion PDE with constant boundary conditions.
    
    This class implements a PDE with non-symmetric diffusion matrix in divergence form,
    featuring a known analytical solution and constant boundary conditions.
    """
    
    def __init__(self, backend=torch):
        """
        Initialize the non-symmetric diffusion PDE.
    ...
    def __repr__(self):
        """
        String representation of the NonSymDiffusion class.
        ...
        return f"NonSymDiffusion(backend={backend_str})"
